...
class AredocoModel {
  ...
  Future<bool> fetchUserPermissionList() async {
    ...
    final permissions = userPermissionDocs.documents
        .map((userPermissionDoc) => Permission(userPermissionDoc))
        .toList();

    this.permissions = permissions;

    debugPrint(
        'fetchUserPermissionList().length= ' + permissions.length.toString());

    for (int i = 0; i < permissions.length; i++) {
      debugPrint('fetchUserPermissionList()= ' +
          permissions[i].homeInformationId +
          ' / ' +
